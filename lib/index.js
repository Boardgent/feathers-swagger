...
const init = module.exports = function (config) {
  return function () {
    ...
    const { docsPath = '/docs', docsJsonPath, docsVersionPath = '', uiIndex, appProperty = 'docs', openApiVersion = 2 } = config;
    ...
    app.get(docsPath, function (req, res) {
      res.format({
        ...
        'text/html': function () {
          ...
          console.log('base1', req.baseUrl);
          console.log('pathname1', pathname);
          if (pathname[pathname.length - 1] !== '/') {
            console.log('base', `${req.baseUrl}${docsVersionPath}${pathname}/`);
            parsed.pathname = `${req.baseUrl}${docsVersionPath}${pathname}/`;
          ...
          if (typeof uiIndex === 'function') {
            ...
            if (req.query.url) {
              ...
              res.redirect(`?url=${encodeURI(`${docsVersionPath}${docsPath}`)}`);
            }
          } else {
            res.json(specs);
          }
